...
export const DetailsHeaderPanel = ({
  ...
  const filteredContent = panelContent?.filter((item) => item?.content && item.content !== 'N A')
  ...
  return (
    filteredContent && filteredContent.length > 0 && (
      ...
      >
        {filteredContent.map((item, index, array) => (
          <div className="flex flex-col gap-3 lg:flex-row lg:inline-flex" key={index}>
